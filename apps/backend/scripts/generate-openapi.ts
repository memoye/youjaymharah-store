...
import { ROUTES, TAGS, type RouteDoc } from "./openapi-routes";

/**
 * Writes docs/api/openapi.json for this project's custom API routes.
 *
 *   pnpm run docs:openapi           regenerate the spec
 *   pnpm run docs:openapi -- --check  fail if it is out of date (CI)
 *
...
function main() {
  ...
  if (check) {
    ...
    if (current !== spec) {
      console.error(
        "docs/api/openapi.json is out of date. Run `pnpm run docs:openapi`.",
      );
      process.exit(1);
    }
  ...
  console.log(
    `Wrote ${relative(resolve(__dirname, "../../.."), OUTPUT)} (${ROUTES.length} operations).`,
  );
}
