...
export type CartReminderEmailProps = {
  /** 1 for the first reminder. */
  reminder_number: number;
  total_reminders: number;
  items: CartReminderItem[];
  /** Opens the storefront with this cart restored. */
  cart_url: string;
  /** Stops cart reminders for this address. */
  stop_url: string;
  brand?: BrandSummary;
};
...
export function cartReminderCopy(
...
function CartReminderEmailComponent({
  reminder_number,
  total_reminders,
  items,
  cart_url,
  ...
  const copy = cartReminderCopy(reminder_number, total_reminders, storeName);
...
export const cartReminderEmail = (props: CartReminderEmailProps) => (
  <CartReminderEmailComponent {...props} />
);

export default function CartReminderEmailPreview() {
  return (
    <CartReminderEmailComponent
      reminder_number={1}
      total_reminders={3}
      items={[
        {
      ...
      cart_url="https://example.com/shopping-bag/restore?token=example"
