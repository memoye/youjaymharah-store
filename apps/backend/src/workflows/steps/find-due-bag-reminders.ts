...
export type DueBagReminder = {
  ...
  /** Null until the bag's first reminder creates its row. */
  reminder: { id: string; token: string; failed_attempts: number } | null;
  items: DueBagReminderItem[];
};

export type FindDueBagRemindersOutput = {
  due: DueBagReminder[];
...
/**
 * Every abandoned bag with a reminder due now. Read-only: sending and status
 * changes happen in the next step.
 *
 * A bag is skipped when:
 * - reminders are off, or it has no email or no items;
 * - its address used a stop link, or unsubscribed from the newsletter;
 * - an order was placed with that address after the bag's last change, since
 *   the shopper most likely bought elsewhere (another device, say);
 * - it was already stopped, recovered, finished or failed.
 */
export const findDueBagRemindersStep = createStep(
  "find-due-bag-reminders",
  async (_input: void, { container }) => {
    const output: FindDueBagRemindersOutput = { due: [], finished_ids: [] };
    ...
    const [reminders, optOuts, unsubscribed, orders] = await Promise.all([
      service.listBagReminders({ cart_id: candidates.map((cart) => cart.id) }),
      service.listBagReminderOptOuts({ email: emails }, { select: ["email"] }),
