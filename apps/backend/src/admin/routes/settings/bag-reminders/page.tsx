...
const SETTINGS_KEY = ["bag-reminders", "settings"];
const STATS_KEY = ["bag-reminders", "stats"];
...
const BagRemindersPage = () => {
  ...
  const settingsQuery = useQuery({
    ...
    queryFn: () =>
      sdk.client.fetch<{ settings: BagReminderSettings }>(
        "/admin/bag-reminders/settings",
      ),
  });
  ...
  const statsQuery = useQuery({
    queryKey: STATS_KEY,
    queryFn: () =>
      sdk.client.fetch<{ stats: BagReminderStats }>(
        "/admin/bag-reminders/stats",
      ),
  });
        ...
        {settingsQuery.isLoading ? (
          ...
          <>
            ...
            <EditDrawer
              ...
              onSaved={(updated) =>
                queryClient.setQueryData<{ settings: BagReminderSettings }>(
                  SETTINGS_KEY,
                  { settings: updated },
                )
              }
      ...
      <Container className="divide-y p-0">
        ...
        {statsQuery.isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-4 px-6 py-4 md:grid-cols-4">
            <Stat label="Bags reminded" value={String(stats.bags_reminded)} />
            <Stat
              label="Bags reopened from an email"
              value={String(stats.bags_opened)}
            />
            <Stat
...
const EditDrawer = ({
  ...
  settings: BagReminderSettings;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSaved: (settings: BagReminderSettings) => void;
}) => {
  ...
  const field = (
    ...
    return (
      <div className="flex flex-col gap-y-2">
        <Label htmlFor={`bag-reminder-${key}`} size="small" weight="plus">
          {label}
        </Label>
        <div className="flex items-center gap-x-2">
          <Input
      ...
      <Drawer.Content>
        ...
        <Drawer.Body className="flex flex-col gap-5 overflow-y-auto">
          <div className="flex items-start justify-between gap-x-4 rounded-lg border border-ui-border-base px-4 py-3">
            <div className="flex flex-col gap-y-1">
              <Label htmlFor="bag-reminders-enabled" size="small" weight="plus">
                Send bag reminders
              </Label>
              <Text
                size="small"
...
const Stat = ({
  ...
  <div className="flex flex-col gap-y-1 rounded-lg border border-ui-border-base px-4 py-3">
    <Text size="small" leading="compact" className="text-ui-fg-subtle">
      {label}
    </Text>
...
export default BagRemindersPage;
